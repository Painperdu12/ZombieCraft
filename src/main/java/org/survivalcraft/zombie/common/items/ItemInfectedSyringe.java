...
public class ItemInfectedSyringe extends Item {
	
	public ItemInfectedSyringe() {
		this.setTextureName("zombiecraft:infectedSyringe");
		this.setUnlocalizedName("infectedSyringe");
	...
	public ItemStack onEaten(ItemStack stack, World world, EntityPlayer player) {
		...
		if(!world.isRemote) {	
			if(!data.isInfected()) {
				...
				world.playSoundAtEntity(player, "zombiecraft:player.cough", 2.5f, 1f);
			...
			if(data.isInfected()) ChatHelper.sendMessage(player, "&cYou are already infected!");
			else ChatHelper.sendMessage(player, "&aYou are now infected!");
		}
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List strings, boolean advanced) {
		strings.add(ChatHelper.getFormattedMessage("&aMedical Item - For Gaining Infection"));
	}
}