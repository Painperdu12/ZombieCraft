...
public class ClientVariables {
	...
	public static float getCustomFogDensityByIndex(int index) {
		if(index == EnumWeatherType.NORMAL.index) return fogNormalDensity;
		else if(index == EnumWeatherType.RAIN.index) return fogRainDensity;
		else if(index == EnumWeatherType.STORM.index) return fogStormDensity;
		else if(index == EnumWeatherType.FOGGY.index) return fogFoggyDensity;
